...
<body>
	<form method="get" action="mailboxDeleteForm" name="mailboxdelete"
		...
		<div class="container" id="mailboxForm"
			...
			<div class="row">
				<div class="col-md-12">
					...
					<div class="row">
						<input type="text" value="삭제편지함-(${rowCount})"
					...
					<div class="row">
						...
						<div class="col-md-6"
							...
							<div class="btn-group" style="height: 33px; float: left;">
								...
								<button type="button" id="deletesave" 
									class="btn btn-success" style="float: left;">삭제취소</button>
								<button type="button" id="deleteend" class="btn btn-danger"
									style="float: left;">메일삭제</button>
								<button type="button" id="deleteall" class="btn btn-dark"
									style="float: left;">비우기</button>
							</div>
						</div>
						<div class="col-md-1"
							style="display: table-cell; height: 33px; float: left;">
								...
								<tr style="text-align: center">
									...
									<td class="text-middle"><c:choose>
											<c:when test="${emailboxs.m_save eq 'Y'}">
												<a href="#"
													onClick="importartOff('${emailboxs.m_seq}'); return false;"><i
													class="fas fa-star"></i></a>
											</c:when>
											<c:when test="${emailboxs.m_save eq 'N'}">
												<a href="#"
													onClick="importartOn('${emailboxs.m_seq}'); return false;"><i
													class="far fa-star"></i></a>
											</c:when>
										</c:choose></td>
									<c:choose>
										<c:when test="${emailboxs.m_fromemail eq sessionemail}">
											<td class="text-middle">${emailboxs.m_fromname}</td>
											<td class="text-middle">${emailboxs.m_date}</td>
											<td class="text-middle"><a
												href="mailDetail?m_seq=${emailboxs.m_seq}"><button
														disabled type="button" class="btn btn-secondary"
														style="border: none; display: inline; width: 60px; font-size: 1em; height: 30px">sent</button>${emailboxs.m_title}</a></td>
										</c:when>
										<c:when
											test="${emailboxs.m_toemail eq sessionemail || emailboxs.m_cc eq sessionemail}">
											<td class="text-middle">${emailboxs.m_toname}</td>
											<td class="text-middle">${emailboxs.m_date}</td>
											<td class="text-middle"><a
												href="mailDetail?m_seq=${emailboxs.m_seq}">${emailboxs.m_title}</a></td>
										</c:when>
									</c:choose>
				...
				<div class="col-sm-12">
					...
					[<a href="mailboxDeleteSelected?page=${page}&find=${find}">${page }</a>]
					</c:forEach>
				</div>

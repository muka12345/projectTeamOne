...
$(document).ready(function() {
	...
	var Proyieldtable = $('#proyield').DataTable({
		dom : 'Bfrtip',
		// 표시 건수기능 숨기기
		lengthChange : false,
		// 정렬 기능 숨기기
		...
		buttons : [ {
			...
			customize : function(xlsx) {
				var sheet = xlsx.xl.worksheets['공정별수율.xml'];
	...
	Proyieldtable.buttons().container().appendTo($('#excelExport'));
	...
	var modeltable = $('#modelexcel').DataTable({
		...
		buttons : [ {
			...
			className : 'btn btn-dark',
			customize : function(xlsx) {
				var sheet = xlsx.xl.worksheets['모델별수율.xml'];
				// jQuery selector to add a border
				$('row c[r*="10"]', sheet).attr('s', '25');
			}
		} ],
